...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return {
    title: locale === 'es'
      ? 'Estores en Torrevieja | A medida con Instalación'
      : 'Blinds in Torrevieja | Custom with Installation',
    description: locale === 'es'
      ? 'Instalación profesional de estores enrollables, cortinas y screen a medida en Torrevieja. Presupuesto gratuito y sin compromiso. Servicio rápido en toda la ciudad.'
      : 'Professional installation of roller blinds, curtains and screen blinds in Torrevieja. Free, no-obligation quote. Fast service across the city.',
    alternates: {
      canonical: `${baseUrl}/${locale}/torrevieja`,
      languages: {
        'es': `${baseUrl}/es/torrevieja`,
        'en': `${baseUrl}/en/torrevieja`,
      },
    },
  };
}

export default async function TorreviejaPage({params}: {params: Promise<{locale: string}>}) {
  const {locale} = await params;
  setRequestLocale(locale);

  const t = await getTranslations('Torrevieja');
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white">
      ...
      <section className="relative h-[80vh] min-h-[600px] flex items-center justify-center text-center px-4 overflow-hidden">
        <Image
          src="/images/hero_estores_torrevieja.jpg"
          alt={t('h1')}
          fill
