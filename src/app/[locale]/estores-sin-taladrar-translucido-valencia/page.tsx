...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return {
    title: locale === 'es' ? `Estor sin Taladrar Translúcido Valencia | Estores Alicante` : `No-Drill Valencia Translucent Blind | Blinds in Alicante`,
    description: locale === 'es' 
      ? `Estor translúcido modelo Valencia sin taladrar la ventana. Instalación rápida sin agujeros por clips, privacidad alta a medida en Alicante.`
      : `Valencia translucent blind installed without drilling. Quick hole-free setup via clips, high custom privacy in Alicante.`,
    openGraph: {
      url: `${process.env.NEXT_PUBLIC_BASE_URL || 'https://estoresalicante.com'}/${locale}/estores-sin-taladrar-translucido-valencia`,
      images: [{ url: BG_IMAGE, width: 800, height: 600, alt: 'estores-sin-taladrar-translucido-valencia' }]
    }
  };
}
...
export default async function Page({params}: {params: Promise<{locale: string}>}) {
  ...
  const product = products.find(p => p.slug === 'estores-sin-taladrar-translucido-valencia');
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "Product",
            "name": locale === 'es' ? "Estor sin taladrar Translúcido Valencia" : "No-Drill Valencia Translucent Blind",
            "image": `https://estoresalicante.com/images/estores-traslucidos-alicante.jpg`,
            "description": locale === 'es' ? "Estor translúcido modelo Valencia sin taladrar la ventana. Instalación rápida sin agujeros por clips, privacidad alta a medida en Alicante." : "Valencia translucent blind installed without drilling. Quick hole-free setup via clips, high custom privacy in Alicante.",
            "offers": {
              "@type": "Offer",
              "priceCurrency": "EUR",
              "price": "16.50",
              "availability": "https://schema.org/InStock",
              "url": `https://estoresalicante.com/${locale}/estores-sin-taladrar-translucido-valencia`
            },
            "aggregateRating": {
      ...
      <section className="relative h-[55vh] min-h-[400px] flex items-center justify-center text-center px-4 overflow-hidden">
        <Image
          src={BG_IMAGE}
          alt="Estor sin taladrar Translúcido Valencia"
          fill
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-white space-y-6">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight leading-tight">
            {locale === 'es' ? 'Estor sin Taladrar Translúcido Valencia' : 'No-Drill Valencia Translucent Blind'}
          </h1>
          <p className="text-lg md:text-xl text-gray-200 max-w-2xl mx-auto font-medium">
            {locale === 'es' 
              ? 'Luz tamizada y privacidad absoluta instalada directamente en el marco de tu ventana. Ideal para inquilinos y pisos de alquiler.'
              : 'Soft diffused light and complete privacy installed directly on your window frame. Ideal for renters and temporary homes.'}
          </p>
          <div className="pt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <h2 className="text-3xl font-extrabold text-[#1A1A2E]">
              {locale === 'es' ? 'Estor sin taladro con tejido Translúcido Valencia' : 'No-Drill Valencia Translucent Roller Blind'}
            </h2>
            <p className="text-gray-700 leading-relaxed">
              {locale === 'es' 
                ? 'El estor enrollable sin taladrar Translúcido Valencia se sujeta firmemente al perfil de tu ventana de aluminio o PVC usando prácticos soportes regulables. Olvídate de taladros y agujeros. Ofrece una privacidad alta con visibilidad nula al exterior.'
                : 'The Valencia Translucent no-drill blind attaches securely to your aluminum or PVC window sash using adjustable clip brackets. No drilling, no tools required. It offers high privacy with zero outdoor visibility.'}
            </p>
            <ul className="space-y-3">
              {[
                ...
                locale === 'es' ? 'Tejido translúcido lavable de alta duración' : 'Highly durable, easy-to-clean translucent fabric'
              ].map((item, idx) => (
