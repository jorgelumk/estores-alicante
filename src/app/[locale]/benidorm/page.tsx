...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return {
    title: locale === 'es'
      ? 'Estores en Benidorm | A medida con Instalación'
      : 'Blinds in Benidorm | Custom with Installation',
    description: locale === 'es'
      ? 'Instalación profesional de estores enrollables, cortinas y screen a medida en Benidorm. Presupuesto gratuito y sin compromiso. Servicio rápido en toda la ciudad.'
      : 'Professional installation of roller blinds, curtains and screen blinds in Benidorm. Free, no-obligation quote. Fast service across the city.',
    alternates: {
      canonical: `${baseUrl}/${locale}/benidorm`,
      languages: {
        'es': `${baseUrl}/es/benidorm`,
        'en': `${baseUrl}/en/benidorm`,
      },
    },
  };
}

export default async function BenidormPage({params}: {params: Promise<{locale: string}>}) {
  const {locale} = await params;
  setRequestLocale(locale);

  const t = await getTranslations('Benidorm');
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white">
      ...
      <section className="relative h-[80vh] min-h-[600px] flex items-center justify-center text-center px-4 overflow-hidden">
        <Image
          src="/images/hero_estores_benidorm.jpg"
          alt={t('h1')}
          fill
