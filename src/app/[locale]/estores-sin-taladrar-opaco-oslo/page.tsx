...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return {
    title: locale === 'es' ? `Estor sin Taladrar Opaco Oslo | Estores Alicante` : `No-Drill Oslo Blackout Blind | Blinds in Alicante`,
    description: locale === 'es' 
      ? `Estor opaco modelo Oslo sin taladrar la ventana. Bloqueo total de luz sin agujeros para dormitorios, confeccionado a medida en Alicante.`
      : `Oslo blackout blind installed without drilling. Total light block with no holes for bedrooms, made to measure in Alicante.`,
    openGraph: {
      url: `${process.env.NEXT_PUBLIC_BASE_URL || 'https://estoresalicante.com'}/${locale}/estores-sin-taladrar-opaco-oslo`,
      images: [{ url: BG_IMAGE, width: 800, height: 600, alt: 'estores-sin-taladrar-opaco-oslo' }]
    }
  };
}
...
export default async function Page({params}: {params: Promise<{locale: string}>}) {
  ...
  const product = products.find(p => p.slug === 'estores-sin-taladrar-opaco-oslo');
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white">
      ...
      <section className="relative h-[55vh] min-h-[400px] flex items-center justify-center text-center px-4 overflow-hidden">
        <Image
          src={BG_IMAGE}
          alt="Estor sin taladrar Opaco Oslo"
          fill
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-white space-y-6">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight leading-tight">
            {locale === 'es' ? 'Estor sin Taladrar Opaco Oslo' : 'No-Drill Oslo Blackout Blind'}
          </h1>
          <p className="text-lg md:text-xl text-gray-200 max-w-2xl mx-auto font-medium">
            {locale === 'es' 
              ? 'Ausencia total de luz solar e intimidad máxima instalada directamente en el marco de tu ventana. Ideal para dormitorios sin persianas.'
              : 'Complete solar light blocking and maximum privacy installed directly on your window frame. Ideal for bedrooms without shutters.'}
          </p>
          <div className="pt-4">
      ...
      <section className="py-12 bg-gray-50 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            ...
            { label: locale === 'es' ? 'Bloqueo: 100%' : '100% Light Block', icon: '🌙' },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <h2 className="text-3xl font-extrabold text-[#1A1A2E]">
              {locale === 'es' ? 'Estor sin taladro con tejido Opaco Oslo' : 'No-Drill Oslo Blackout Roller Blind'}
            </h2>
            <p className="text-gray-700 leading-relaxed">
              {locale === 'es' 
                ? 'El estor enrollable sin taladrar Opaco Oslo se sujeta firmemente al perfil de tu ventana de aluminio o PVC usando prácticos soportes regulables. Olvídate de taladros y agujeros. Ofrece una privacidad total con bloqueo de luz absoluta del 100%.'
                : 'The Oslo Blackout no-drill blind attaches securely to your aluminum or PVC window sash using adjustable clip brackets. No drilling, no tools required. It offers total privacy with absolute 100% light block.'}
            </p>
            <ul className="space-y-3">
              {[
                ...
                locale === 'es' ? 'Tejido opaco técnico multicapa de alta duración' : 'Highly durable, technical multi-layer blackout fabric'
              ].map((item, idx) => (
          ...
          <div className="bg-gray-50 p-8 rounded-2xl border border-gray-100 space-y-4">
            ...
            <div className="space-y-2 text-sm text-gray-700">
              ...
              <div className="flex justify-between border-b py-1"><strong>{locale === 'es' ? 'Privacidad:' : 'Privacy:'}</strong><span>{locale === 'es' ? 'Total (Noche y Día)' : 'Absolute (Day & Night)'}</span></div>
