...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return {
    title: locale === 'es' ? `Estor sin Taladrar Screen 5% a Medida | Estores Alicante` : `No-Drill Screen 5% Blind | Blinds in Alicante`,
    description: locale === 'es' 
      ? `Estor screen 5% sin taladrar la ventana. Instalación rápida sin agujeros por clips, a medida al mejor precio de Alicante.`
      : `Screen 5% blind installed without drilling. Quick hole-free setup via clips, custom sizes at the best price in Alicante.`,
    openGraph: {
      url: `${process.env.NEXT_PUBLIC_BASE_URL || 'https://estoresalicante.com'}/${locale}/estores-sin-taladrar-screen-5`,
      images: [{ url: BG_IMAGE, width: 800, height: 600, alt: 'estores-sin-taladrar-screen-5' }]
    }
  };
}
...
export default async function Page({params}: {params: Promise<{locale: string}>}) {
  ...
  const product = products.find(p => p.slug === 'estores-sin-taladrar-screen-5');
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white">
      ...
      <section className="relative h-[55vh] min-h-[400px] flex items-center justify-center text-center px-4 overflow-hidden">
        <Image
          src={BG_IMAGE}
          alt="Estor sin taladrar Screen 5%"
          fill
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-white space-y-6">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight leading-tight">
            {locale === 'es' ? 'Estor sin Taladrar Screen 5%' : 'No-Drill Screen 5% Blind'}
          </h1>
          <p className="text-lg md:text-xl text-gray-200 max-w-2xl mx-auto font-medium">
            {locale === 'es' 
              ? 'Protección solar screen de alta densidad instalada directamente en el marco de tu ventana. Ideal para inquilinos y pisos de alquiler.'
              : 'High density screen solar protection installed directly on your window frame. Ideal for renters and temporary homes.'}
          </p>
          <div className="pt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <h2 className="text-3xl font-extrabold text-[#1A1A2E]">
              {locale === 'es' ? 'Estor sin taladro con tejido Screen 5%' : 'No-Drill Screen 5% Roller Blind'}
            </h2>
            <p className="text-gray-700 leading-relaxed">
              {locale === 'es' 
                ? 'El estor enrollable sin taladrar Screen 5% se sujeta firmemente al perfil de tu ventana de aluminio o PVC usando prácticos soportes regulables. Olvídate de taladros y agujeros. Ofrece una protección solar media (5% de apertura) con visibilidad óptima al exterior.'
                : 'The Screen 5% no-drill blind attaches securely to your aluminum or PVC window sash using adjustable clip brackets. No drilling, no tools required. It offers medium sun protection (5% openness) with good outdoor visibility.'}
            </p>
